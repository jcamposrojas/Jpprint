...
class ProductoGenerico(models.Model):
    _name = 'producto_generico'
    _description = 'Producto Generico'

    company_id       = fields.Many2one('res.company', default=lambda self: self.env.company)
    cost_currency_id = fields.Many2one('res.currency', readonly=True, related='company_id.currency_id')
    producto_id      = fields.Many2one("cotizador.producto", string="Producto", required=True)
...
#            self.uom_id_de_consumo = None
    ...
    def create(self,vals):
        producto_id = self.env.context.get('producto_id',False)
        if producto_id:
            vals['producto_id'] = producto_id

        return super(ProductoGenerico, self).create(vals)
